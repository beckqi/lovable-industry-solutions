      ...
      <div className="max-w-[1600px] mx-auto px-6">
        ...
        <div className="flex gap-3 h-[480px]">
          {caseStudies.map((item, index) => (
            <motion.div key={item.id} onMouseEnter={() => setActiveIndex(index)} animate={{ flex: activeIndex === index ? 4 : 0.6 }} transition={{ duration: 0.5 }} className={`relative rounded-3xl overflow-hidden cursor-pointer ${activeIndex === index ? "shadow-xl" : "shadow-sm"}`}>
              <div className={`absolute inset-0 bg-gradient-to-br ${item.gradient}`} />
              <AnimatePresence>
                {activeIndex !== index && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 flex items-center justify-center"><h3 className="text-xl font-bold text-primary" style={{ writingMode: "vertical-rl" }}>{item.industry}</h3></motion.div>)}
              </AnimatePresence>
              <AnimatePresence>
                {activeIndex === index && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ delay: 0.1 }} className="relative h-full p-8 flex gap-8">
                    <div className="flex flex-col w-[280px] flex-shrink-0">
                      <h3 className="text-2xl font-bold text-primary mb-6">{item.industry}</h3>
                      <div className="flex-1 rounded-2xl bg-white/60 backdrop-blur-sm border border-white/80 shadow-lg flex items-center justify-center"><div className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center"><span className="text-2xl font-bold text-primary">{item.logo}</span></div></div>
                    </div>
